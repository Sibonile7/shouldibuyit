...
function renderRecallsHTML(recalls) {
        ...
        return `
            <div class="recalls-section">
                ...
                <div class="section-explainer">
                    A recall is when the manufacturer confirmed a safety defect and will fix it for free at any dealer.
                </div>
    ...
    const sectionId = `recalls-more-${recallSectionCounter}`;
    const visible = recalls.slice(0, 2);
    const hidden = recalls.slice(2);
    const visibleHTML = visible.map(renderSingleRecallCard).join('');
    const hiddenHTML = hidden.map(renderSingleRecallCard).join('');
    return `
        <div class="recalls-section">
            <div class="samples-title">${recalls.length} official recall${recalls.length === 1 ? '' : 's'}</div>
            <div class="section-explainer">
                A recall means the manufacturer confirmed a safety defect in this car.
                The fix is free at any authorized dealer. If buying used, ask if these were completed.
            </div>
            ${visibleHTML}
            ${hidden.length > 0 ? `
                <div id="${sectionId}" class="recalls-hidden" style="display:none;">
                    ${hiddenHTML}
                </div>
                <button class="load-more-btn" onclick="toggleRecalls('${sectionId}', this)">
                    Show ${hidden.length} more recall${hidden.length === 1 ? '' : 's'}
                </button>
            ` : ''}
        </div>
    `;
...
function toggleRecalls(sectionId, btn) {
    const section = document.getElementById(sectionId);
    if (section.style.display === "none") {
        section.style.display = "block";
        btn.textContent = "Show fewer recalls";
    } else {
        section.style.display = "none";
        const count = section.querySelectorAll('.recall-card').length;
        btn.textContent = `Show ${count} more recall${count === 1 ? '' : 's'}`;
    }
}
...
function renderAlignedComparison(cars) {
    ...
    const globalMax = Math.max(
        ...allComponents.map(c => Math.max(issues1[c] || 0, issues2[c] || 0)), 1
    );
    ...
    const alignedRows = allComponents.map((comp, i) => {
        ...
        const percent1 = count1 > 0 ? Math.round((count1 / total1) * 100) : 0;
        const percent2 = count2 > 0 ? Math.round((count2 / total2) * 100) : 0;
        const barClass = `bar-${Math.min(i + 1, 10)}`;
        return `
            <div class="aligned-row">
                <div class="aligned-cell aligned-left">
                    <div class="aligned-count ${count1 === 0 ? 'zero' : ''}">${count1} <span class="issue-pct">(${percent1}%)</span></div>
                ...
                <div class="aligned-cell aligned-right">
                    ...
                    <div class="aligned-count ${count2 === 0 ? 'zero' : ''}">${count2} <span class="issue-pct">(${percent2}%)</span></div>
                </div>
            </div>
        `;
    }).join('');
    ...
    return `
        <div class="compare-aligned">
            ...
            <div class="section-explainer" style="text-align:center;">
                Each row shows the same component for both cars. Bigger bar = more complaints.
            </div>
            ${alignedRows}

            <!-- COMPLAINTS: side by side, same height with load more -->
            <div class="compare-details-grid">
                <div class="compare-detail-card" id="${cardId1}">
                    <div class="samples-title">${esc(car1.make)} ${esc(car1.model)} complaints</div>
                ...
                <div class="compare-detail-card" id="${cardId2}">
                    <div class="samples-title">${esc(car2.make)} ${esc(car2.model)} complaints</div>
                    <div class="section-explainer">Reports filed by owners to the US government.</div>
            ...
            <!-- RECALLS: full width, side by side headers but stacked cards -->
            <div class="compare-recalls-section">
                <div class="aligned-section-title">Official recalls</div>
                <div class="section-explainer" style="text-align:center;">
                    A recall means the manufacturer confirmed a safety defect. The fix is free at any dealer.
                </div>
                <div class="compare-recalls-grid">
                    <div class="compare-recall-col">
                        <div class="compare-recall-header">${esc(car1.make)} ${esc(car1.model)}: ${recalls1} recall${recalls1 === 1 ? '' : 's'}</div>
                        ${recalls1 === 0
                            ? '<div class="no-recalls">No recalls found.</div>'
                            : car1.recalls.slice(0, 2).map(renderSingleRecallCard).join('')
                              + (car1.recalls.length > 2 ? `
                                <div id="compare-recalls-1" style="display:none;">${car1.recalls.slice(2).map(renderSingleRecallCard).join('')}</div>
                                <button class="load-more-btn" onclick="toggleRecalls('compare-recalls-1', this)">Show ${car1.recalls.length - 2} more recall${car1.recalls.length - 2 === 1 ? '' : 's'}</button>
                              ` : '')
                        }
                    </div>
                    <div class="compare-recall-col">
                        <div class="compare-recall-header">${esc(car2.make)} ${esc(car2.model)}: ${recalls2} recall${recalls2 === 1 ? '' : 's'}</div>
                        ${recalls2 === 0
                            ? '<div class="no-recalls">No recalls found.</div>'
                            : car2.recalls.slice(0, 2).map(renderSingleRecallCard).join('')
                              + (car2.recalls.length > 2 ? `
                                <div id="compare-recalls-2" style="display:none;">${car2.recalls.slice(2).map(renderSingleRecallCard).join('')}</div>
                                <button class="load-more-btn" onclick="toggleRecalls('compare-recalls-2', this)">Show ${car2.recalls.length - 2} more recall${car2.recalls.length - 2 === 1 ? '' : 's'}</button>
                              ` : '')
                        }
                    </div>
                </div>
...
async function loadAllComplaints(make, model, year, cardId) {
    ...
    const url = `/api/car/${make}/${model}/${year}/component/ALL?limit=20&offset=0`;
    const res = await fetch(url);
    if (!res.ok) {
        const fallbackRes = await fetch(`/api/car/${make}/${model}/${year}`);
        const fallbackData = await fallbackRes.json();
        samplesList.innerHTML = (fallbackData.sample_complaints || [])
            .map(s => renderSampleHTML(s, null)).join('');
        return;
    }
    const data = await res.json();
    filterState[cardId] = {
        make, model, year, component: "ALL",
        offset: data.complaints.length, total: data.total_matching,
    };
    samplesList.innerHTML = data.complaints.map(c => renderSampleHTML(c, null)).join('');
    if (data.has_more) {
...
async function filterByComponent(make, model, year, component, cardId) {
    ...
    if (headerEl) {
        headerEl.innerHTML = `
            <div class="samples-title">${data.total_matching} ${esc(component)} complaint${data.total_matching === 1 ? '' : 's'}</div>
            <button class="reset-filter" onclick="resetFilter('${esc(make)}', '${esc(model)}', ${year}, '${esc(cardId)}')">Show all</button>
        `;
    }
    samplesList.innerHTML = data.complaints.map(c => renderSampleHTML(c, component.toUpperCase())).join('');
    if (data.has_more) {
        const loadMore = document.createElement("button");
        loadMore.className = "load-more-btn";
        loadMore.textContent = `Load more (${data.total_matching - data.complaints.length} remaining)`;
...
async function resetFilter(make, model, year, cardId) {
    ...
    samplesList.innerHTML = data.sample_complaints.map(s => renderSampleHTML(s, null)).join('');
